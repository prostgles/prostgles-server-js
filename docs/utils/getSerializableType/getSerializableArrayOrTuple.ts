...
export const getSerializableArrayOrTuple: TsTypeParser = ({
  ...
  const arrayOrTuple = checker.isArrayType(myType)
    ? "array"
    : checker.isTupleType(myType)
      ? "tuple"
      : undefined;
  if (arrayOrTuple) {
    const itemType =
      arrayOrTuple === "tuple"
        ? myType
        : checker.getTypeArguments(myType as ts.TypeReference)[0];
    if (itemType && checker.isTupleType(itemType)) {
      const tupleTypes =
        (
          itemType as unknown as { resolvedTypeArguments: ts.Type[] }
        ).resolvedTypeArguments.map((d: ts.Type) => {
    ...
    const resolvedItemType: TS_Type = itemType
      ? getSerializableType({
          myType: itemType,
          checker,
          visited,
