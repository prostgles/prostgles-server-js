    ...
    */
     ...
     * */
    {
        ...
        getQuery: ({ allowedFields, args, tableAlias }) => {
            ...
            if (typeof edgeTruncate !== "number")
                throw "Invalid edgeTruncate. expecting number";
            if (typeof noFields !== "boolean")
                throw "Invalid noFields. expecting boolean";
            let col = "( " + cols.map(c => `${noFields ? "" : (asValue(c + ": ") + " || ")} COALESCE(${exports.asNameAlias(c, tableAlias)}::TEXT, '')`).join(" || ', ' || ") + " )";
            term = asValue(term);
            if (!matchCase) {
                col = "LOWER" + col;
                term = `LOWER(${term})`;
            }
            let leftStr = `substr(${col}, 1, position(${term} IN ${col}) - 1 )`, rightStr = `substr(${col}, position(${term} IN ${col}) + length(${term}) )`;
            if (edgeTruncate > -1) {
