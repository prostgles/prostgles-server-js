...
export default async function client_files(db: Required<DBHandlerClient>, auth: Auth, log: (...args: any[]) => any, methods, tableSchema: DBSchemaTable[]){
  ...
  let insertedFile;
  await tryRun("Insert file from nested insert", async () => {
  ...
  await tryRun("Cannot Insert file directly", async () => {
    try {
  ...
  await tryRun("Can update allowed files directly", async () => {
    const newData =  { 
  ...
  await tryRun("Can insert allowed files through a nested update", async () => {
  ...
  await tryRun("Can delete only allowed files directly", async () => {
