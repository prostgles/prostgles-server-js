...
const util_1 = require("prostgles-types/util");
;
...
class ViewHandler {
    ...
    async buildJoinQuery(q) {
        const makeQuery3 = (q, depth = 0, joinFields) => {
            const PREF = `prostgles`, joins = q.joins || [], aggs = q.aggs || [], makePref = (q) => !q.joinAlias ? q.table : `${q.joinAlias || ""}_${q.table}`, makePrefANON = (joinAlias, table) => exports.asName(!joinAlias ? table : `${joinAlias || ""}_${table}`), makePrefAN = (q) => exports.asName(makePref(q));
            ...
            const prefJCAN = (q, str) => exports.asName(`${q.joinAlias || q.table}_${PREF}_${str}`);
            // const indent = (a, b) => a;
            const joinTables = (q1, q2) => {
                ...
                return Prostgles_1.flat(paths.map(({ table, on }, i) => {
                    ...
                    let iQ = [
                        exports.asName(table) + ` ${thisAlias}`
                    ];
                    ...
                    let jres = [
                        ...
                        ...iQ,
                        `ON ${on.map(([c1, c2]) => `${prevAlias}.${exports.asName(c1)} = ${thisAlias}.${exports.asName(c2)} `).join(" AND ")}`
                    ];
                    return jres;
                }));
            };
            /* Leaf query */
            if (!joins.length) {
                let select = (depth ? q.allFields : q.select), groupBy = "";
                ...
                if (q.aggs && q.aggs.length) {
                    ...
                    if (q.select.length) {
                        groupBy = `GROUP BY ${groupByFields.concat((q.selectFuncs || []).map(sf => exports.asName(sf.alias))).join(", ")}\n`;
                    }
                }
                // let res = "" +
                // `SELECT -- leaf query\n` + 
            ...
            if (q.selectFuncs.length)
                throw "Functions within select not allowed in joins yet. -> " + q.selectFuncs.map(s => s.alias).join(", ");
            const rootSelect = [
                " ",
                `-- 0. [root final]  `,
                "SELECT    ",
    ...
    getAggs(select) {
        ...
        let aliased = keys.filter(key => isPlainObject(select[key]))
            ...
            .filter((f) => f.parser)
            .map((a) => {
            let data = select[a.alias][a.parser.name];
    ...
    async buildQueryTree(filter, selectParams, param3_unused = null, tableRules, localParams) {
        ...
        if (isPlainObject(select)) {
            ...
            joinAliases = Object.keys(select)
                .filter(key => !aggAliases.includes(key) &&
                (select[key] === "*"
                    ||
            ...
            if (joinAliases && joinAliases.length) {
                ...
                for (let i = 0; i < joinAliases.length; i++) {
                    let jKey = joinAliases[i], jParams = select[jKey], jTable = jKey, isLeftJoin = true, jSelectAlias = jKey, jSelect = jParams, jFilter = {}, jLimit = undefined, jOffset = undefined, jOrder = undefined, jPath = undefined;
                    ...
                    if (isLocal || joinTableRules) {
                        const joinQuery = await this.dboBuilder.dbo[jTable].buildQueryTree(jFilter, { select: jSelect, limit: jLimit, offset: jOffset, orderBy: jOrder, alias: jSelectAlias }, param3_unused, joinTableRules, localParams);
            ...
            mainSelect = PubSubManager_1.filterObj(select, Object.keys(select).filter(key => !(aggAliases.concat(joinAliases).includes(key))));
            /* Allow empty select */
        ...
        if (ambiguousAggName)
            throw `Cannot have select columns collide with aggregation alias for: ` + ambiguousAggName;
        q.joins = joinQueries;
        q.aggs = _Aggs;
        q.joinAlias = alias;
        return q;
    ...
    async getColumns(tableRules, localParams) {
    ...
    async subscribe(filter, params, localFunc, table_rules, localParams) {
        try {
            ...
            const { filterFields, forcedFilter } = utils_1.get(table_rules, "select") || {}, condition = await this.prepareWhere(filter, forcedFilter, filterFields, true, null, localParams, table_rules);
            if (!localFunc) {
    ...
    async prepareWhere(filter, forcedFilter, filterFields, excludeWhere = false, tableAlias = null, localParams, tableRule) {
        const parseFilter = async (f, parentFilter = null) => {
            let result = "";
    ...
    async prepareExistCondition(eConfig, localParams, tableRules) {
        ...
        let finalWhere;
        try {
            finalWhere = await this.dboBuilder.dbo[t2].prepareWhere(f2, forcedFilter, filterFields, true, tableAlias, localParams, tableRules);
        }
        catch (err) {
    ...
    /* NEW API !!! :) */
    async getCondition(filter, allowed_colnames, tableAlias, localParams, tableRules) {
        let prefix = "";
        ...
        if (rowHashCondition)
            templates.push(rowHashCondition);
        ...
        try {
            ...
            if (patchedTextData && patchedTextData.length) {
                ...
                patchedTextData.map(p => {
                    data[p.fieldName] = util_1.unpatchText(rows[0][p.fieldName], p.from, p.to, p.text, p.md5);
                });
                // https://w3resource.com/PostgreSQL/overlay-function.php
