...
export async function addSync(
  ...
  const res = await tryCatchV2(async () => {
    ...
    const upsertSync = () => {
      ...
      */
      const handlers = ReplicationProtocol.getServerHandlers(channelName, socket, {
        ClientSyncRequest: async (data) => {
          const err = await this.syncData(newSync, data, "client")
            .then(() => undefined)
            .catch((err) => {
              console.error("Error syncing data with client: ", err);
              return { success: false, err: getSerialisableError(err) } as const;
            });

          return err ?? ({ success: true } as const);
        },
      });
      const newSync = {
