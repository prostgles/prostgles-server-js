...
class Prostgles {
    ...
    async getUserFromCookieSession(socket) {
        // console.log("conn", socket.handshake.query, socket._session)
        const sid = this.getSID(socket);
        if (!sid)
            return null;
...
class PublishParser {
    ...
    async getValidatedRequestRuleWusr({ tableName, command, socket }) {
        const user = await this.prostgles.getUser(socket);
        return await this.getValidatedRequestRule({ tableName, command, socket }, user);
    }
    async getValidatedRequestRule({ tableName, command, socket }, user) {
        if (!this.dbo)
    ...
    async getTableRules({ tableName, socket }, user) {
        try {
            if (!socket || !tableName)
