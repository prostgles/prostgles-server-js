...
export function makeSelectQuery(
  ...
  if (!joins.length) {
    ...
    const simpleQuery = indentLinesToString(depth, [
      `-- 0. or 5. [leaf query] `
  ...
  const joinedQuery = [
    ...
    , `-- 0. [joined root]  `
    , "SELECT    "
    , ...insertCommas(rootSelectItems.map(s => s.getQuery() + " AS " + asName(s.alias)).concat(
      parsedJoins.map((j, i) => {
  ...
  const res = indentLinesToString(depth, joinedQuery as unknown as string[]);
