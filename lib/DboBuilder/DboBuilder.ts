...
export class DboBuilder {
  ...
  tableTsDefinitions?: string;
  functionTsDefinitions?: string;
  joinGraph?: Graph;
  ...
  getTsDefinitions = <DDL extends string | undefined = undefined>({
    ...
    extraTables?: TableSchema[];
    /**
     * Optionally provide a TRUSTED DDL statement obtain an updated schema after applying the DDL.
     * It is crucial that the statement is trusted and does not container commits.
     ...
     */
    ddlWithRollback?: DDL;
  } = {}): DDL extends string ? Promise<string> : string => {
    const getSchema = (tablesOrViews: TableSchema[]) => {
      ...
      return [
        ...
        tableTsDefinitions,
        excludeFunctions ? "" : this.functionTsDefinitions,
      ]
        .filter((v) => v)
        .join("\n");
    };
    if (ddlWithRollback) {
      return new Promise<string>((resolve, reject) => {
        getTablesForSchemaPostgresSQL(this, {
          ...
          .then((res) => resolve(getSchema(res.result)))
          .catch(reject);
      }) as DDL extends string ? Promise<string> : string;
    }
    const tablesOrViews = this.tablesOrViews;
    if (!tablesOrViews) {
      throw new Error("Unexpected error: tablesOrViews is undefined");
    }
    return getSchema(tablesOrViews) as DDL extends string ? Promise<string> : string;
  };
